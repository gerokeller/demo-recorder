...
async function teardownPass(pass: PassRuntime): Promise<{ videoPath: string | undefined }> {
...
async function main(): Promise<void> {
  ...
  try {
    ...
    const stepBeats = scenario.steps.map((s) => ('beat' in s ? (s.beat ?? null) : null));
    const stepEmphases = scenario.steps.map((s) => ('emphasis' in s ? (s.emphasis ?? null) : null));
